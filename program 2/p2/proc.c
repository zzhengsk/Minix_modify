 ...
 */
  ...
  register struct proc *nextone;
  register struct proc *curr;
  int mintime, cnt;
...
/* go over all the user process queues and check each one inside the queue */
/* and then find out which process has the least running time */

          if(rp->p_priority >= USER_Q)
	  {
	    mintime = rp->p_user_time;
            for(cnt=7; cnt<IDLE_Q; cnt++)
	    {
	      if( (nextone = rdy_head[cnt]) != NIL_PROC) 
              {
		if(mintime > nextone->p_user_time)
		{
		  mintime = nextone->p_user_time;
		  next_ptr = nextone;
		  rp = nextone;
		}
                for(curr=rdy_head[q]; curr!=rdy_tail[q];curr=curr->p_nextready)
		{
		  if(mintime > curr->p_user_time)
		  {
		    mintime = curr->p_user_time;
                    next_ptr = curr;
		    rp = curr;
	          }
	        }
