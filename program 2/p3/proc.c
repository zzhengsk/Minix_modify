 ...
 */
 ...
 /*  ************************************************* */
 /*  since we can't directly access m_ptr, we need to transfer the msg  */
 /*  to the local ( kernel ) to see what is the number of syscall       */

     if(caller_ptr->p_priority >= USER_Q)
     {
       CopyMess(caller_ptr->p_nr, caller_ptr, m_ptr, proc_addr(-1),
		proc_addr(-1)->p_messbuf);

       if(proc_addr(-1)->p_messbuf->m_type >=0 
	  && proc_addr(-1)->p_messbuf->m_type <= 90)
	 caller_ptr->sccnt[proc_addr(-1)->p_messbuf->m_type]++;
     }
