 ...
 */
  register struct hole *hp, *prev_ptr, *maxhp;
  ...
  do {
	...
	while(hp!=NIL_HOLE && hp->h_next!=NIL_HOLE && hp->h_base< swap_base)
	{
		if(maxhp->h_len < (hp->h_next)->h_len)
		{
			maxhp = hp->h_next;
			maxhp->h_next = (hp->h_next)->h_next;
	      		prev_ptr = hp;
			prev_ptr->h_next = maxhp;
		}
		hp = hp->h_next;
 	}
