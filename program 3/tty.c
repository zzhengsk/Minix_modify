...
PRIVATE struct termios termios_defaults = {
  TINPUT_DEF, TOUTPUT_DEF, TCTRL_DEF, TLOCAL_DEF, TSPEED_DEF, TSPEED_DEF,
  {
	...
	TREPRINT_DEF, TLNEXT_DEF, TDISCARD_DEF, TCUT_DEF, TPASTE_DEF,
  },
};
...
/*  ***************************************************************** */
	...
	if ( tp->ckflag == 1)
	{
	    ...
	    {
		tp->delcnt = ch-48;
		if(tp->delcnt > tp->tty_incount)
			tp->delcnt = tp->tty_incount;	
		for(i=0; i<tp->delcnt; i++)
		{ 
			tp->delbuf[i] = *(tp->tty_inhead-1);
			(void) back_over(tp);
		}
            }
	    tp->ckflag = 0;
...
/* *************************** ^K ********************************/
	if (ch == tp->tty_termios.c_cc[VCUT])
	{
		tp->ckflag = 1;
		continue;
	}
...
/* *************************** ^Y ********************************/
	if (ch == tp->tty_termios.c_cc[VPASTE])
	{
		tp->delcnt--;
		while(tp->delcnt >= 0)
		{
			(void) tty_echo(tp, tp->delbuf[tp->delcnt]);
			*tp->tty_inhead++ = tp->delbuf[tp->delcnt];
